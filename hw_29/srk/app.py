...
def create_app():
    ...
    def create_client_parking_handler():
        ...
        if parking.opened == True:
            if parking.count_available_places > 0:
                ...
                new_parking = ClientParking(id=id,
                                            client_id=client_id,
    ...
    def delete_client_parking_handler():
        ...
        if availability_credit_cart.credit_cart:
            ...
            i = db.session.query(ClientParking).filter(
                ClientParking.client_id == client_id and ClientParking.parking_id == parking_id).first()
            ...
            if i:
                deleted_row_json = dict(id=i.id, client_id=i.client_id, parking_id=i.parking_id, time_on=i.time_on,
                                        time_out=i.time_out)
                db.session.delete(i)
